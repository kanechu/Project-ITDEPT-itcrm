...
-(BOOL)fn_save_com_sys_code:(NSString*)sys_code{
    if ([[idb fn_get_db]open]) {
        BOOL isDelete=[[idb fn_get_db]executeUpdate:@"delete from com_sys_code where sys_code =?",sys_code];
        if (!isDelete) {
            return NO;
        }
        NSString *insertSql=[NSString stringWithFormat:@"insert into com_sys_code(sys_code)values(\"%@\")",sys_code];
...
-(NSMutableArray*)fn_get_com_sys_code{
    ...
    if ([[idb fn_get_db]open]) {
        FMResultSet *lfmdb_result=[[idb fn_get_db]executeQuery:@"select * from com_sys_code"];
