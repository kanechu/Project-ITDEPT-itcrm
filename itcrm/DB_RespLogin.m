...
-(BOOL)fn_save_data:(NSMutableArray*)arr{
    if ([[idb fn_get_db] open]) {
        for (RespLogin *lmap_data in arr) {
            ...
            BOOL ib_delete =[[idb fn_get_db] executeUpdate:@"delete from loginInfo where company_code = :company_code and sys_name = :sys_name and env = :env and web_addr =:web_addr" withParameterDictionary:ldict_row];
            if (! ib_delete)
                return NO;
            BOOL ib_updated =[[idb fn_get_db] executeUpdate:@"insert into loginInfo (company_code, sys_name, env, web_addr) values (:company_code, :sys_name, :env, :web_addr)" withParameterDictionary:ldict_row];
            if (! ib_updated)
                return NO;
...
-(BOOL)fn_delete_all_data{
    if ([[idb fn_get_db]open]) {
      BOOL isSuccess=[[idb fn_get_db]executeUpdate:@"delete from loginInfo"];
        if (!isSuccess) {
            return NO;
        }
        return YES;
