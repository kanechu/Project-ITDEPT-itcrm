...
-(NSMutableArray*)fn_get_relate_crmopp_data:(NSString *)opp_ref_id select_sql:(NSString *)select_sql{
    ...
    NSString *is_sql=[NSString stringWithFormat:@"select %@ from crmopp_browse where opp_ref_type = 'ACCT' and opp_ref_id like ? ORDER BY rec_upd_date,rec_crt_date DESC",select_sql];
    __block NSMutableArray *arr=[NSMutableArray array];
    [queue inDataBase:^(FMDatabase *db){
        if ([db open]) {
