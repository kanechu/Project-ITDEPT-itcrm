...
-(BOOL)fn_update_crmopp_ismodified:(NSString*)is_modified unique_id:(NSString*)unique_id{
    if ([[idb fn_get_db]open]) {
        BOOL ib_updated =[[idb fn_get_db]executeUpdate:@"update crmopp_browse set is_modified=? where unique_id=?",[NSString stringWithFormat:@"%@",is_modified],[NSString stringWithFormat:@"%@",unique_id]];
        if (!ib_updated)
            return NO;
        [[idb fn_get_db] close];
        return  YES;
...
-(NSMutableArray*)fn_get_relate_crmopp_data:(NSString *)opp_ref_id select_sql:(NSString *)select_sql{
    select_sql=[select_sql stringByAppendingString:@",opp_id"];
