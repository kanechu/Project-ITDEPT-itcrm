...
-(NSMutableArray*)fn_get_MaintForm_data:(NSString*)form_id{
    ...
    [queue inDataBase:^(FMDatabase *db){
        ...
        if ([db open]) {
            FMResultSet *lfmdb_result=[db executeQuery:@"SELECT * FROM maintForm where form_id = ?",form_id];
...
-(NSMutableArray*)fn_get_groupNameAndNum:(NSString*)form_id{
    ...
    [queue inDataBase:^(FMDatabase *db){
        if ([db open]) {
            FMResultSet *lfmdb_result= [db executeQuery:@"SELECT group_name,min(seq) as seq,COUNT(group_name) FROM maintForm where form_id = ? group by group_name order by seq ASC",form_id];
            while ([lfmdb_result next]) {
